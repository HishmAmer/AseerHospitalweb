...
class Nationality(models.Model):
    ...
    # ترتيب العرض. القائمة الأصلية مرتّبة حسب المنطقة لا أبجدياً («سعودي» أولاً)،
    # وحفظ الترتيب يُبقي القائمة مألوفة لمن يُدخل البيانات. المضاف حديثاً يُلحق
    # بالنهاية عبر save() أدناه، فلا يقفز إلى أعلى القائمة.
    ...
    def save(self, *args, **kwargs):
        if not self.display_order:
            last = Nationality.objects.order_by('-display_order').first()
...
class Employee(models.Model): 
    ...
    # PROTECT لا SET_NULL: حذف جنسية مستخدَمة يجب أن يُرفض برسالة واضحة بدل
    # أن يفرّغ الحقل بصمت في سجلات موظفين قائمة.
    nationality = models.ForeignKey(
        'Nationality', on_delete=models.PROTECT, null=True,
