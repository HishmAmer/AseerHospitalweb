# نظام إدارة الكادر الطبي | تجمع عسير الصحي

تطبيق Django لإدارة بيانات الكادر الطبي والإجازات والصلاحيات عبر منشآت التجمع.
...
- `RENDER_EXTERNAL_HOSTNAME` متاح **وقت التشغيل فقط** (وللخدمات من نوع web).
  لذلك لا يشترط التطبيق وجود `ALLOWED_HOSTS` في أوامر البناء (`collectstatic`
  و `migrate`) لأنها لا تستقبل طلبات، بينما يرفض `gunicorn` و `runserver`
  الإقلاع بدونه. أي أن البناء ينجح والحماية تبقى سارية عند تقديم الطلبات.
